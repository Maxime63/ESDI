...
public final class SocpEx1 {
   ...
   /* Solve the model extracted to CPLEX and test KKT conditions on the solution.
   ...
   private static boolean checkkkt (IloCplex cplex,
      ...
      Map<IloNumVar,Double> dj = new TreeMap<IloNumVar,Double>(AddableCompare);
      Map<IloNumVar,Double> x  = new TreeMap<IloNumVar,Double>(AddableCompare);
      Map<IloRange,Double> pi = new TreeMap<IloRange,Double>(AddableCompare);
      ...
      //       read those multipliers directly from the reduced cost values
      //       returned for the cone head variables. For a second order cone
      //       constraint
      //         x[1] >= |(x[2], ..., x[n])|
      //       the dual multiplier is the reduced cost value returned for x[1].
      //       This is used in recordVar() to set the dual multiplier for second
      //       order cone constraints.
      for (IloNumVar v : vars) {
         x.put(v, cplex.getValue(v));
         dj.put(v, cplex.getReducedCost(v));
         if ( cone.get(v) != NOT_CONE_HEAD && cone.get(v) != NOT_IN_CONE )
            pi.put(cone.get(v), cplex.getReducedCost(v));
      }
      for (IloRange r : rngs) {
         if ( !pi.containsKey(r) )
      ...
      out.println(" ]");
      out.print("dj     = [");
      for (IloNumVar v : dj.keySet())
         out.format(" %7.3f", dj.get(v));
      out.println(" ]");
      out.print("pi     = [");
      ...
      // - the dj value for all non-cone variables must be non-negative.
      // Note that we do not support ranged constraints here.
      for (IloNumVar v : vars) {
         if ( cone.get(v) == NOT_IN_CONE && dj.get(v) < -tol ) {
            err.println("Dual multiplier for " + v + " is not feasible: " + dj.get(v));
            return false;
         }
      }
      for (IloRange r : rngs) {
      ...
      for (IloNumVar v : vars) {
         if ( cone.get(v) == NOT_IN_CONE ) {
            if ( Math.abs(x.get(v)) > tol && dj.get(v) > tol ) {
               err.println("Invalid complementary slackness for " + v + ":");
               err.println(" " + x.get(v) + " and " + dj.get(v));
               return false;
            }
         }
      }
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         cplex.setParam(IloCplex.DoubleParam.BarQCPEpComp, CONVTOL);
 
         // Solve model and test the KKT conditions on the solution.
